...
- **Step 4 — Auto-refresh (เสร็จ):** `.github/workflows/auto-refresh.yml` รัน `npm run import` (incremental gviz) ตาม cron `30 1 * * *` (01:30 UTC = 08:30 ไทย) ทุกวัน → ดึง Data_Date ใหม่เข้าเองโดยไม่ต้องกด Import · reuse ทุก path (summary rebuild + auto-prune อัตโนมัติ) · ไม่มีวันใหม่ = no-op ราคาถูก · repo เป็น public → Actions ฟรีไม่จำกัด · **ต้องตั้งค่าครั้งเดียวโดยคน:** GitHub repo → Settings → Secrets and variables → Actions → เพิ่ม **Secret `DATABASE_URL`** (= Supabase connection string) · (ไม่บังคับ) **Variable `SHEET_URL`** ถ้าไม่ตั้งจะใช้ default sheet ในสคริปต์ · ทดสอบ: Actions tab → "Auto refresh data" → Run workflow
...
| 2026-08-03 | **✅ RESOLVED: ย้าย DB Neon→Supabase + กู้ข้อมูลครบ + แก้ต้นเหตุ** — Supabase (`qrrdqsgpksmxsavdeubo`, Sydney, ฟรี) มีครบ **12 snapshots (97,789 แถว)**: 8 แรกจากไฟล์เครื่อง `CM WO Tracking...xlsx` sheet "Follow up backlog CM ", 4 ล่าสุดดึงจาก Neon (โควตารีเซ็ต) · trend กลับมา +30.0% · **cache 2 ชั้น** (`draft-state.ts` commit `c0625f3`) ลด egress ~99% · prisma.ts ตั้ง ssl rejectUnauthorized:false · **Neon เลิกใช้ถาวร** · งานค้าง = ตั้ง Render env `DATABASE_URL`=Supabase |
